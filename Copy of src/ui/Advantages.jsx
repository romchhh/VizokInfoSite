...
function Advantages() {
  ...
  return (
    <section
      ...
      id='advantages'
    >
      <h2 className='text-5xl font-bold text-black'>Наші переваги</h2>
      <div
        ref={advantagesRef}
      ...
      >
        {ADVANTAGES.map((advantage, index) => (
          <div className='advantage-item' key={index}>
            <Advantage
              src={advantage.img}
              title={<span className="text-2xl text-black">{advantage.title}</span>}
              index={index + 1}
            >
              <p className="text-2xl text-black">{advantage.description}</p>
            </Advantage>
          </div>
        ))}
      </div>
